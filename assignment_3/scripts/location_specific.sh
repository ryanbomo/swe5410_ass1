...
for ((II=0; II < ${#GENERATORS[@]}; ++II)) do
  ...
  echo "" >> $ERRORNAME
  ssh $TESTSITE $TESTPATH${GENERATORS[II]} $2 $3  | $LOC${MYPROGS[II]} >> $LOGNAME 2>>$ERRORNAME & echo "$!"
  top  -n1 -p $!
  ps o pid,ppid,comm,%cpu,%mem,m_trs,m_drs,rss,vsz,sz $!
  wait
  echo "" >> $LOGNAME
  echo "" >> $ERRORNAME
  ...
  echo "" >> $ERRORNAME
  ssh $TESTSITE $TESTPATH${GENERATORS[II]} $2 $3  | ssh $TESTSITE $ORACLEPATH${TARGETS[II]} >> $LOGNAME 2>>$ERRORNAME & echo "$!"
  top  -n1 -p $!
  ps o pid,ppid,comm,%cpu,%mem,m_trs,m_drs,rss,vsz,sz $!
  wait
  echo "" >> $LOGNAME
